...
![NVDA Scenario Lab：已保存的合成演示证据、蜡烛图、成交量与预测区间](docs/images/scenario-lab-demo.png)

*2026-09-15 从本地演示截取的 Scenario Lab。图中价格与预测均为合成数据，
用于展示产品流程，不作为真实行情或投资表现证据。*
...
3. 展开**风险与决策（Risk & decision）**，填写**决策理由（Decision reason）**，
   点击 **Watch decision / Reject decision** 保存分析。如要演练模拟交易，
   先检查提案，仅在证据和风控允许时确认。
4. 重载已保存决策包的网址，展开 **Monitoring & review**，
   核对原始依据和已有结果；尚未观测到的结果保持待定。
...
| 阶段 | 结果 |
| --- | --- |
| 已交付：基础与工作站，0001–0020 | 确定性模拟内核、研究/数据适配器、风控审计、React 工作站、本地化和标的决策工作区。 |
