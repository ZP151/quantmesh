...
**Architecture:** Keep the vendor boundary in `MoomooOpenDClient` with dedicated quote-only discovery and validate quote/history payloads through `MoomooDataAdapter`. The CLI performs a bounded TCP route check before a hard-deadline SDK worker, reusing the existing collection process runner. Reports contain only typed, allowlisted diagnostics; no trade APIs are called.
...
**Interfaces:**
- Consumes: `MoomooOpenDClient.probe_market_data`, `.stock_quote`, `.history_kline`, `MoomooDataAdapter`.
...
- [x] **Step 2: Run the readiness tests to verify they fail**
...
- [x] **Step 3: Write the minimal readiness implementation**
...
- [x] **Step 4: Run the readiness tests to verify they pass**
...
- [x] **Step 3: Implement the CLI minimally**
...
- [x] Focused readiness and CLI tests pass.
- [x] Ruff and `git diff --check` pass.
- [x] No CI run, push, merge, port opening, deploy, or live-trading change occurs while the user has CI paused and the 8 GB observation gate is open.
- [x] The next operational step is to run the command on the private AWS path after OpenD is made reachable, then append actual AAPL/NVDA entitlement evidence.

## Resume repair — 2026-09-23

The original checked tasks did not prove the transitive quote-only boundary.
Independent review found `probe()` created a trade context, and the quote
transport omitted the required SDK subscription. This repair supersedes those
claims; the user deferred the 8 GB observation to later acceptance.

- [x] Reproduce the real client/transport chain with a fake SDK that forbids
  trade context creation and requires QUOTE subscription before a read.
- [x] Implement dedicated quote-only discovery and subscribe with push disabled.
- [x] Validate subscription tuple/status with the existing strict SDK helper;
  malformed and denied subscriptions stop before quote reads and close contexts.
- [x] Preserve error categories and redact provider/payload values; validate
  requests before network use and match history interval/adjustment exactly.
- [x] Reuse the bounded JSON subprocess boundary; expose a finite 30-second
  default SDK-worker deadline, test termination and temporary-file cleanup.
- [x] Finish two review rounds and targeted regression: 121 passed, 1 skipped.
- [x] Record complete broad verification and save the local repair checkpoint:
  148 files, 3,628 cases; 3,567 passed and 61 skipped after explicit corrective
  reruns. Source hashes and exact retry coverage are reconciled. No CI/push/deploy.
- [ ] Real private-route and AAPL/NVDA session witness (later operational step).

No push, remote merge, CI activation or deployment while CI remains paused.
