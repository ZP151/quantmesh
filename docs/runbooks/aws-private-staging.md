...
This runbook creates one private QuantMesh acceptance station for a
...
private `/api/health`, the active release symlink and the existing systemd unit;
retain the previous release. The 2026-09-12 initial probe found `4022942`, demo,
paper true and live trading false. The commands below do not purchase data,
change Tailscale configuration or create AWS resources.

After downloading the candidate's deployment script/unit to a temporary
directory (as in section 3), install the candidate unit before activation:

```bash
sudo install -m 0644 "${QM_BOOTSTRAP_DIR}/quantmesh-staging.service" \
  /etc/systemd/system/quantmesh-staging.service
sudo systemctl daemon-reload
sudo python3 "${QM_BOOTSTRAP_DIR}/deploy_release.py" "$QM_COMMIT" \
  --origin "$QM_ORIGIN" --live-market-data
```

The unit defaults to demo and reads the release's canonical environment.
Only the explicit flag selects `--live`, BTC/ETH/SOL and persistent
`/var/lib/quantmesh/live/{data,orders,decisions}`. Healthy activation requires
the exact build plus `runtime_mode=live`, `paper_mode=true`,
...
Keep missing historical charts/forecasts explicitly unavailable.

To roll back, use the reviewed deployment script with the retained exact SHA:

```bash
sudo python3 "${QM_BOOTSTRAP_DIR}/deploy_release.py" \
  402294248406fa865d601633f4e5ba3bd3521b5b \
  --origin "$QM_ORIGIN" --activate-existing
```

Do not pass `--live-market-data` with retained activation: the saved profile
is authoritative. The old release's unchanged env selects default demo;
