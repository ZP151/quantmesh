...
export function CommandPalette({
  ...
  const items = useMemo<PaletteItem[]>(() => {
    const screens: PaletteItem[] = NAV_ITEMS.map((item) => ({
      key: `nav:${item.path}`,
      label: item.label,
      hint: `Go to ${item.group}`,
      run: () => navigate(item.path),
    }))
    const actions: PaletteItem[] = [
      { key: 'ks-engage', label: 'Engage kill switch', hint: 'Refuse every paper and live order', run: () => void killSwitch.mutate(true) },
      { key: 'ks-disarm', label: 'Disarm kill switch', hint: 'Re-allow paper orders', run: () => void killSwitch.mutate(false) },
      ...(demoAttached
        ? [
            { key: 'demo-reset', label: 'Reset demo session', hint: 'Restore the pristine seeded root', run: () => void resetDemo.mutate() },
          ]
        : []),
    ]
  ...
  }, [query, navigate, killSwitch, resetDemo, demoAttached])
      ...
      <Dialog.Portal>
        ...
        <Dialog.Popup className="fixed top-[15vh] left-1/2 z-50 w-[min(36rem,calc(100vw-2rem))] -translate-x-1/2 rounded-xl border border-border bg-popover text-popover-foreground shadow-2xl outline-none">
          ...
          <ul className="max-h-72 overflow-y-auto p-1.5" role="listbox" aria-label="Commands">
            {items.length === 0 && (
              <li className="px-3 py-4 text-sm text-muted-foreground">No screens or actions match “{query}”.</li>
            )}
          ...
          <div className="flex items-center justify-between border-t border-border px-3 py-1.5 text-[10px] text-muted-foreground">
            <span>
              {demoAttached ? 'Paper demo session attached' : 'Operator mode — no demo session'}
            </span>
            <span className="flex items-center gap-1">
              <RotateCcw className="size-3" aria-hidden /> writes refetch every screen
            </span>
          </div>
        </Dialog.Popup>
      </Dialog.Portal>
